...
def get_file_name(year: int, month: int, day: int) -> str:
    ...
    """
    file_name = f"smard_{year}_{month:02d}_{day:02d}.parquet"
    return f"raw/smard/year={year}/month={month:02d}/{file_name}"

def upload_to_s3(dataframe: pd.DataFrame):
    ...
    file_name = get_file_name(year, month, day)
